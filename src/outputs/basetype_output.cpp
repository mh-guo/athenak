...
BaseTypeOutput::BaseTypeOutput(ParameterInput *pin, Mesh *pm, OutputParameters opar) :
  ...
  // pgen-specific: xray_binary cell-by-cell heating rate (Scherbak Eq. 7)
  if (out_params.variable.compare("xrb_heat") == 0) {
    out_params.contains_derived = true;
    out_params.n_derived += 1;
    int i_derived = out_params.n_derived - 1;
    outvars.emplace_back("heat",i_derived,&(derived_var));
  }
