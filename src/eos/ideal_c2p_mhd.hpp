...
void SingleC2P_IdealMHD(MHDCons1D &u, const EOS_Data &eos,
  ...
  // apply entropy floor
  Real lg_sfloor_local = log10(eos.sfloor1) + (log10(w.d)-log10(eos.rho1)) *
...
Real Equation44(const Real mu, const Real b2, const Real rpar, const Real r, const Real q,
  ...
  Real lg_sfloor_local = log10(eos.sfloor1) + (log10(wd)-log10(eos.rho1)) *
        (log10(eos.sfloor2)-log10(eos.sfloor1))/(log10(eos.rho2)-log10(eos.rho1));
  Real sfloor_local = pow(10.0, lg_sfloor_local);
...
void SingleC2P_IdealSRMHD(MHDCons1D &u, const EOS_Data &eos, Real s2, Real b2, Real rpar,
  ...
  Real lg_sfloor_local = log10(eos.sfloor1) + (log10(dens)-log10(eos.rho1)) *
        (log10(eos.sfloor2)-log10(eos.sfloor1))/(log10(eos.rho2)-log10(eos.rho1));
  Real sfloor_local = pow(10.0, lg_sfloor_local);
  sfloor_local = fmax(eos.sfloor, sfloor_local);
