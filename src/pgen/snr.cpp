...
void AddScalarForOneCloud(Mesh *pm, ParameterInput *pin);
...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  Real rout_snr = rout;
  psnr.snr_tend = pin->GetOrAddReal("problem","snr_tend",0.0);
  psnr.snr_dt   = pin->GetOrAddReal("problem","snr_dt",-1.0);
  psnr.snr_next = static_cast<Real>((static_cast<int>(pmy_mesh_->time/psnr.snr_dt) + 1)
                  *psnr.snr_dt);
  ...
  psnr.mu_h = pin->GetOrAddReal("problem","mu_h",1.0);
  ...
  psnr.vol_snr = vol_snr;
  psnr.dens_snr = dens_snr;
  psnr.eint_snr = eint_snr;
  ...
  if (global_variable::my_rank == 0) {
    ...
    std::cout << "  snr_dt = " << psnr.snr_dt << std::endl;
    std::cout << "  snr_next = " << psnr.snr_next << std::endl;
    std::cout << "  snr_tend = " << psnr.snr_tend << std::endl;
  }
  // End print info
  ...
  if (rst_flag) {
    ...
    par_for("pgen_snr_uni", DevExeSpace(),0,(pmbp->nmb_thispack-1),ks,ke,js,je,is,ie,
  ...
  // setup spherical over-pressured region and passive scalars in Hydro
  if (pmbp->phydro != nullptr) {
    ...
    bool mark_cloud = pin->GetOrAddBoolean("problem","mark_cloud",false);
    if (mark_cloud) {
      if (global_variable::my_rank == 0) {
        std::cout << " Adding scalar..." << std::endl;
      }
      pmbp->phydro->ConToPrim(nullptr, 0);
      AddScalarForOneCloud(pmy_mesh_, pin);
      std::cout << " Adding scalar complelted" << std::endl;
    }
...
void UserWorkInCycle(Driver *pdriver, Mesh *pm) {
  if (psnr.snr_dt>0.0 && pm->time > psnr.snr_next && pm->time < psnr.snr_tend) {
    ...
    if (global_variable::my_rank == 0) {
      std::cout << "AddSNR" << std::endl;
    }
    AddSNR(pm);
    auto phydro = pm->pmb_pack->phydro;
    (void) phydro->RestrictU(pdriver, 0);
    (void) phydro->InitRecv(pdriver, -1);  // stage < 0 suppresses InitFluxRecv
    (void) phydro->SendU(pdriver, 0);
    (void) phydro->ClearSend(pdriver, -1);
    (void) phydro->ClearRecv(pdriver, -1);
    (void) phydro->RecvU(pdriver, 0);
    (void) phydro->ApplyPhysicalBCs(pdriver, 0);
    (void) phydro->ConToPrim(pdriver, 0);
    (void) phydro->NewTimeStep(pdriver, pdriver->nexp_stages);
    phydro->dtnew*=0.3;
  }
}
...
void AddSNR(Mesh *pm) {
  ...
  if (pmbp->phydro != nullptr) {
    ...
    int &ks = indcs.ks; int &ke = indcs.ke; int nx3 = indcs.nx3;
    Real rout     = psnr.rout;
    Real dens_snr = psnr.dens_snr;
    Real eint_snr = psnr.eint_snr;
    auto &u0 = pmbp->phydro->u0;
    int &nhydro = pmbp->phydro->nhydro;
    int &nscalars = pmbp->phydro->nscalars;
    par_for("add_snr", DevExeSpace(),0,(pmbp->nmb_thispack-1),ks,ke,js,je,is,ie,
    KOKKOS_LAMBDA(int m,int k, int j, int i) {
      Real &x1min = size.d_view(m).x1min;
      Real &x1max = size.d_view(m).x1max;
      Real x1v = CellCenterX(i-is, nx1, x1min, x1max);
      ...
      if (rad < rout) {
        u0(m,IDN,k,j,i) = dens_snr;
        u0(m,IM1,k,j,i) = 0.0;
        u0(m,IM2,k,j,i) = 0.0;
        u0(m,IM3,k,j,i) = 0.0;
        u0(m,IEN,k,j,i) = eint_snr;
        // add passive scalars
        if (nscalars > 3) u0(m,nhydro+3,k,j,i) = u0(m,IDN,k,j,i);
      }
    });
...
void AddISMCooling(Mesh *pm, const Real bdt, DvceArray5D<Real> &u0,
  ...
  Real n_h_unit = pmy_pack->punit->density_cgs()/psnr.mu_h
...
void AddScalarForOneCloud(Mesh *pm, ParameterInput *pin) {
  ...
  Real cloud_x = pin->GetOrAddReal("problem", "cloud_x", 0.0);
  Real cloud_y = pin->GetOrAddReal("problem", "cloud_y", 0.0);
  Real cloud_z = pin->GetOrAddReal("problem", "cloud_z", 0.0);

  printf("staring point: %e %e %e %e\n",cloud_x,cloud_y,cloud_z,size.h_view(0).dx1);
  ...
  int &nscalars = pmbp->phydro->nscalars;
  int ns = nhydro+4;
  Real t_c = psnr.t_cold;
  int myrank = global_variable::my_rank;

  int nout1 = indcs.nx1 + 2*(indcs.ng);
  int nout2 = (indcs.nx2 > 1)? (indcs.nx2 + 2*(indcs.ng)) : 1;
  int nout3 = (indcs.nx3 > 1)? (indcs.nx3 + 2*(indcs.ng)) : 1;
  par_for("init", DevExeSpace(),0,(pmbp->nmb_thispack-1),0,nout3-1,0,nout2-1,0,nout1-1,
    KOKKOS_LAMBDA(int m,int k, int j, int i) {
    u0(m,ns,k,j,i)=0.0;
  });

  par_for("findstart", DevExeSpace(),0,(pmbp->nmb_thispack-1),ks,ke,js,je,is,ie,
    KOKKOS_LAMBDA(int m,int k, int j, int i) {
      Real &x1min = size.d_view(m).x1min;
      Real &x1max = size.d_view(m).x1max;
      ...
      if (fabs(cloud_x-x1v)<0.5*size.d_view(m).dx1 &&
          fabs(cloud_y-x2v)<0.5*size.d_view(m).dx2 &&
          fabs(cloud_z-x3v)<0.5*size.d_view(m).dx3) {
        u0(m,ns,k,j,i)=u0(m,IDN,k,j,i);
        printf("staring index: %d %d %d %d %d %f %f %f %e %e %e %e %e %e %e %e\n",myrank,
        m,k,j,i,x1v,x2v,x3v,u0(m,IDN,k,j,i),w0(m,IEN,k,j,i),
        size.d_view(m).dx1,size.d_view(m).dx2,size.d_view(m).dx3,
        abs(cloud_x-x1v),abs(cloud_y-x2v),abs(cloud_z-x3v));
      }
  });
  ...
  while (adding_all) {
    ...
    while (adding) {
      ...
      adding = false;
      Kokkos::parallel_reduce("addscalar", Kokkos::RangePolicy<>(DevExeSpace(), 0, nmkji),
      KOKKOS_LAMBDA(const int &idx, int &sum0) {
        int m = (idx)/nkji;
        int k = (idx - m*nkji)/nji;
        int j = (idx - m*nkji - k*nji)/nx1;
        int i = (idx - m*nkji - k*nji - j*nx1) + is;
        ...
        Real tim = w0(m,IEN,k,j,i-1)/w0(m,IDN,k,j,i-1)*gm1;
        Real tip = w0(m,IEN,k,j,i+1)/w0(m,IDN,k,j,i+1)*gm1;
        Real tjm = w0(m,IEN,k,j-1,i)/w0(m,IDN,k,j-1,i)*gm1;
        Real tjp = w0(m,IEN,k,j+1,i)/w0(m,IDN,k,j+1,i)*gm1;
        Real tkm = w0(m,IEN,k-1,j,i)/w0(m,IDN,k-1,j,i)*gm1;
        Real tkp = w0(m,IEN,k+1,j,i)/w0(m,IDN,k+1,j,i)*gm1;

        Real &sim = u0(m,ns,k,j,i-1);
        Real &sip = u0(m,ns,k,j,i+1);
        Real &sjm = u0(m,ns,k,j-1,i);
        Real &sjp = u0(m,ns,k,j+1,i);
        Real &skm = u0(m,ns,k-1,j,i);
        Real &skp = u0(m,ns,k+1,j,i);

        if ((sim>0.0 || sip>0.0 || sjm>0.0 || sjp>0.0 || skm>0.0 || skp>0.0) &&
            //(tim<t_c || tip<t_c || tjm<t_c || tjp<t_c || tkm<t_c || tkp<t_c) &&
            (u0(m,ns,k,j,i)==0.0) && temp<t_c) {
          u0(m,ns,k,j,i)=u0(m,IDN,k,j,i);
          sum0++;
          //printf("adding: %d %d %d %d %d %e\n",global_variable::my_rank,m,k,j,i,temp);
        }
      }, Kokkos::Sum<int>(sum_0));
      if (sum_0>0) {
        adding = true;
        sum_1 += sum_0;
        printf("Adding: %d %d \n",sum_0,sum_1);
    ...
    int* psum_0 = &(sum_0);
    int* psum_1 = &(sum_1);
    MPI_Allreduce(MPI_IN_PLACE, psum_0, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, psum_1, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
#endif
    sum_2 += sum_1;
    if (global_variable::my_rank == 0) {
      std::cout << " Adding scalar sum_0=" << sum_0 << std::endl
                << " Adding scalar sum_1=" << sum_1 << std::endl
                << " Adding scalar sum_2=" << sum_2 << std::endl;
    }
    if (sum_1>0) {
      adding_all = true;
      sum_1 = 0;
    }
    auto state0 = pmbp->phydro->InitRecv(nullptr, -1);
    auto state1 = pmbp->phydro->SendU(nullptr, 0);
    pmbp->phydro->ClearSend(nullptr, -1);
    pmbp->phydro->ClearRecv(nullptr, -1);
    auto state2 = pmbp->phydro->RecvU(nullptr, 0);
  }
  return;
}
...
void LoadData(Mesh *pm, ParameterInput *pin) {
  ...
  int rst_cut = pin->GetOrAddInteger("problem","rst_cut",0);
  if (rst_cut==1) {
    // temporary version, only for specific number of gpus and mbs
    int rst_cutgids = (((global_variable::my_rank)/2)*6+7)*64+mygids;
    std::cout << global_variable::my_rank << " " << mygids << " " << rst_cutgids
              << std::endl;
    myoffset = headeroffset + (ccdata_cnt+fcdata_cnt)*rst_cutgids*sizeof(Real);
  }
...
// TODO(@mhguo): it doesn't work for nmb>1!!!
void FineToCoarse(Mesh *pm, ParameterInput *pin) {
  std::string rst_file = pin->GetOrAddString("problem", "rst_file", "none");
  MeshBlockPack *pmbp = pm->pmb_pack;
  ParameterInput* pinput = new ParameterInput;
