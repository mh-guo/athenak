...
void FineToCoarse(Mesh *pm, ParameterInput *pin) {
  ...
  for (int itr=0; itr<nitr; itr++) {
    ...
    // on each read call for very large grids per MPI rank
    int mygids = pm->gids_eachrank[global_variable::my_rank];
    int itrgids = mygids*nitr+itr*nmb;
    ...
    // Set initial conditions
    int idmy = itr*nmb;
    for (int jtr=0; jtr<nitr; jtr++) {
      printf("rank=%d i=%d j=%d id=%d nmb=%d n=%d \n",
             global_variable::my_rank,itr,jtr,idmy,nmb,rst_n);
      ...
      KOKKOS_LAMBDA(int m, int k, int j, int i) {
        ...
        int cj = (j-js)/2+js+j0;
        int ci = (i-is)/2+is+i0;
        Real fac = (((jtr/4)%2)^((k-ks)%2))
                   &(((jtr/2)%2)^((j-js)%2))
                   &((jtr%2)^((i-is)%2));
        u0(cm,IDN,ck,cj,ci) += fac*fine_u0(m,IDN,k,j,i)/nitr;
        u0(cm,IM1,ck,cj,ci) += fac*fine_u0(m,IM1,k,j,i)/nitr;
        u0(cm,IM2,ck,cj,ci) += fac*fine_u0(m,IM2,k,j,i)/nitr;
        u0(cm,IM3,ck,cj,ci) += fac*fine_u0(m,IM3,k,j,i)/nitr;
        u0(cm,IEN,ck,cj,ci) += fac*fine_u0(m,IEN,k,j,i)/nitr;
        // add passive scalars
        for (int n=nhydro; n<(nhydro+nscalars); ++n) {
