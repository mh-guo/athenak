...
static void ComputePrimitiveSingle(Real x1v, Real x2v, Real x3v, CoordData coord,
                                   struct gal_pgen pgen,
                                   Real& rho, Real& pgas,
                                   Real& uu1, Real& uu2, Real& uu3);

KOKKOS_INLINE_FUNCTION
static void GetBoyerLindquistCoordinates(struct gal_pgen pgen,
                                         Real x1, Real x2, Real x3,
                                         Real *pr, Real *ptheta, Real *pphi);

KOKKOS_INLINE_FUNCTION
static void TransformVector(struct gal_pgen pgen,
                            Real a1_bl, Real a2_bl, Real a3_bl,
                            Real x1, Real x2, Real x3,
                            Real *pa1, Real *pa2, Real *pa3);

KOKKOS_INLINE_FUNCTION
static void CalculatePrimitives(struct gal_pgen pgen, Real r,
                                Real *prho, Real *ppgas, Real *pur);

KOKKOS_INLINE_FUNCTION
static Real TemperatureMin(struct gal_pgen pgen, Real r, Real t_min, Real t_max);

KOKKOS_INLINE_FUNCTION
static Real TemperatureBisect(struct gal_pgen pgen, Real r, Real t_min, Real t_max);

KOKKOS_INLINE_FUNCTION
static Real TemperatureResidual(struct gal_pgen pgen, Real t, Real r);

KOKKOS_INLINE_FUNCTION
static Real Acceleration(struct gal_pgen pgen, const Real r);
...
void GalFinalWork(ParameterInput *pin, Mesh *pm);
...
//  Compile with '-D PROBLEM=gr_gal' to enroll as user-specific problem generator
...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  pgen_final_func = GalFinalWork;

  // Read problem-specific parameters from input file
  // global parameters
  gal.k_adi = pin->GetReal("problem", "k_adi");
  gal.r_crit = pin->GetReal("problem", "r_crit");

  // Get ideal gas EOS data
  gal.gm = peos->eos_data.gamma;
  Real gm1 = gal.gm - 1.0;

  // Parameters
  gal.temp_min = 1.0e-7;  // lesser temperature root must be greater than this
  gal.temp_max = 1.0e0;   // greater temperature root must be less than this

  // Get spin of black hole
  gal.spin = pmbp->pcoord->coord_data.bh_spin;

  // Get excision parameters
  gal.dexcise = pmbp->pcoord->coord_data.dexcise;
  gal.pexcise = pmbp->pcoord->coord_data.pexcise;

  gal.potential = pin->GetOrAddBoolean("problem", "potential", false);
  gal.sigma2 = pin->GetOrAddReal("problem", "sigma2", 0.0);
  gal.r_iso = pin->GetOrAddReal("problem", "r_iso", 0.0);

  // Get initial condition type
  std::string ic_type = pin->GetOrAddString("problem","ic_type","gr_bondi");
  if (ic_type == "gr_bondi") { gal.ic_type = 0;
  } else if (ic_type == "uniform") { gal.ic_type = 1;
  } else if (ic_type == "entropy") { gal.ic_type = 2;
  } else {
  ...
  gal.n_adi = 1.0/(gal.gm - 1.0);

  // Prepare various constants for determining primitives
  Real u_crit_sq = 1.0/(2.0*gal.r_crit);                           // (HSW 71)
  Real u_crit = -sqrt(u_crit_sq);
  Real t_crit = (gal.n_adi/(gal.n_adi+1.0)
                 * u_crit_sq/(1.0-(gal.n_adi+3.0)*u_crit_sq));     // (HSW 74)
  gal.c1 = pow(t_crit, gal.n_adi) * u_crit * SQR(gal.r_crit);  // (HSW 68)
  gal.c2 = (SQR(1.0 + (gal.n_adi+1.0) * t_crit)
              * (1.0 - 3.0/(2.0*gal.r_crit)));                     // (HSW 69)
  gal.temp_norm = (sqrt(gal.c2)-1.0)/(1.0+gal.n_adi);           // (HSW 69)
  gal.c_s_norm = sqrt(gal.gm * gal.temp_norm);
  gal.r_bondi = 2.0/SQR(gal.c_s_norm);
  ...
  if (gal.ic_type > 0) {
    // Prepare various constants for determining primitives
    gal.temp_norm = pin->GetReal("problem", "temp_norm");
    gal.c_s_norm = sqrt(gal.gm * gal.temp_norm);
    gal.r_bondi = 2.0/SQR(gal.c_s_norm);
    gal.rho_norm = pow(gal.temp_norm/gal.k_adi, gal.n_adi);
    gal.rho_norm = pin->GetOrAddReal("problem", "dens_norm", gal.rho_norm);
    if (gal.cooling) {
      Real t_char = gal.r_bondi / gal.c_s_norm;
      auto punit = pmbp->punit;
      Real temp_unit = punit->temperature_cgs();
      Real n_h_unit = punit->density_cgs()/gal.mu_h/punit->atomic_mass_unit_cgs;
    ...
    gal.k_adi = gal.temp_norm / pow(gal.rho_norm, gm1); // update k_adi
    gal.pgas_norm = gal.rho_norm * gal.temp_norm;
    // gal.r_crit = (5.0-3.0*gal.gm)/4.0;
    // gal.c1 = 0.25*pow(2.0/(5.0-3.0*gal.gm), (5.0-3.0*gal.gm)*0.5*gal.n_adi);
    // gal.c2 = -gal.n_adi; // useless in Newtonian case
  }

  std::string bc_type = pin->GetOrAddString("problem","bc_type","none");
  if (bc_type == "none") { gal.bc_type = 0;
  } else if (bc_type == "fixed") { gal.bc_type = 1;
  } else {
    std::cout << "### ERROR in " << __FILE__ << " at line " << __LINE__ << std::endl
              << "Unknown boundary condition type: " << bc_type << std::endl;
    std::exit(EXIT_FAILURE);
  }
  gal.rb_out = pin->GetOrAddReal("problem","rb_out", std::numeric_limits<Real>::max());

  gal.ndiag = pin->GetOrAddInteger("problem", "ndiag", -1);

  if (global_variable::my_rank == 0) {
    std::cout << " Bondi radius = " << gal.r_bondi << std::endl;
    std::cout << " Critical radius = " << gal.r_crit << std::endl;
    std::cout << " c1 = " << gal.c1 << std::endl;
    std::cout << " rho_norm = " << gal.rho_norm << std::endl;
    if (gal.potential) {
      std::cout << " Potential: sigma2 = " << gal.sigma2 << " r_iso = " << gal.r_iso
                << std::endl;
    }
    if (gal.cooling) {
      std::cout << " t_cool / t_bondi = " << gal.tau_cool << std::endl;
    }
    if (gal.ic_type == 2) {
      std::cout << " Radius of entropy core: r_entropy = " << gal.r_entropy << std::endl;
    }
    std::cout << " Initial condition type  = " << ic_type << std::endl;
    std::cout << " Boundary condition type = " << bc_type << std::endl;
    std::cout << " Outer boundary radius   = " << gal.rb_out << std::endl;
  }
  ...
  par_for("pgen_gal", DevExeSpace(), 0,(nmb-1),0,n3m1,0,n2m1,0,n1m1,
...
void FixedBondiInflow(Mesh *pm) {
  ...
  KOKKOS_LAMBDA(int m, int k, int j, int i) {
    ...
    if (mb_bcs.d_view(m,BoundaryFace::inner_x1) == BoundaryFlag::user) {
      ComputePrimitiveSingle(x1v,x2v,x3v,coord,gal_,rho,pgas,uu1,uu2,uu3);
      w0_(m,IDN,k,j,i) = rho;
      w0_(m,IEN,k,j,i) = pgas/(gal_.gm - 1.0);
      w0_(m,IM1,k,j,i) = uu1;
      w0_(m,IM2,k,j,i) = uu2;
      w0_(m,IM3,k,j,i) = uu3;
    }
    ...
    if (mb_bcs.d_view(m,BoundaryFace::outer_x1) == BoundaryFlag::user) {
      ...
      w0_(m,IEN,k,j,(ie+i+1)) = pgas/(gal_.gm - 1.0);
  ...
  KOKKOS_LAMBDA(int m, int k, int j, int i) {
    ...
    if (mb_bcs.d_view(m,BoundaryFace::outer_x2) == BoundaryFlag::user) {
      ...
      w0_(m,IEN,k,(je+j+1),i) = pgas/(gal_.gm - 1.0);
  ...
  KOKKOS_LAMBDA(int m, int k, int j, int i) {
    ...
    if (mb_bcs.d_view(m,BoundaryFace::outer_x3) == BoundaryFlag::user) {
      ...
      w0_(m,IEN,(ke+k+1),j,i) = pgas/(gal_.gm - 1.0);
...
void BondiFluxes(HistoryData *pdata, Mesh *pm) {
  ...
  if (pm->pzoom != nullptr) {
    pdata->nhist += 1;
    pdata->label[0] = "zone";
    pdata->hdata[0] = (global_variable::my_rank == 0)? pm->pzoom->zstate.zone : 0.0;
  }
  ...
  Real n_h_unit = pmbp->punit->density_cgs()/gal.mu_h/pmbp->punit->atomic_mass_unit_cgs;
  ...
  Real rglb = std::min(gal.rb_out, gal.r_entropy);
...
void AddAccel(Mesh *pm, const Real bdt, DvceArray5D<Real> &u0,
  ...
  KOKKOS_LAMBDA(const int m, const int k, const int j, const int i) {
    ...
    Real accel = Acceleration(gal_, rad);
    if (rad < rin) {
      accel = 0.0;
    }
...
#endif
  if (global_variable::my_rank == 0) {
    if (gal.ndiag>0 && pm->ncycle % gal.ndiag == 0) {
